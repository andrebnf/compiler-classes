...
public class Lexer {
  ...
  public void nextToken() {
    ...
    if (ch == '\0')
      ...
      tokenPos += 2;
      while (!(input[tokenPos] == '*' && input[tokenPos + 1] == '/') || input[tokenPos] == '\0'){
        if (input[tokenPos] == '\0'){
          error.signal("Expected `*/` but EOF found");
        }
        tokenPos++;
      }
      tokenPos += 2;
      ...
      if (isLetter(ch)) {
        ...
        try {
          if (isDouble) {
            ...
            if (doubleValue >= MaxValueDouble)
              error.signal("Double out of limits");
          }
          else {
            token = Symbol.INT;
            intValue = Integer.valueOf(number.toString()).intValue();
            if (intValue >= MaxValueInteger)
              error.signal("Int out of limits");
          }
        } catch (NumberFormatException e) {
          error.signal("Could not convert number");
        }
